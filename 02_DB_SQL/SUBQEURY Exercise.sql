...
from emp e
where (e.hiredate, e.deptno) in (select min(hiredate), deptno
								 from emp
                                 group by deptno);

        
-- 사원 정보에 ename과 더불어 부서 위치(LOC) 출력
select e.ename `사원명`, (select d.loc
				 from dept d
                 where e.deptno = d.deptno) `부서 위치`
from emp e;

 
-- 부서별 인원수와 평균 급여를 가진 가상 테이블을 만들고, 인원수가 5명 이상인 부서만 조회
select e.dept `부서` , e.number `인원수` , e.average `평균 급여`
from (select deptno as dept, count(*) as number, avg(sal) as average
	  from emp 
	  group by deptno) e
where e.number >= 5;
...
-- 직급이 'MANAGER'인 사원들이 속한 부서의 모든 사원 정보를 출력 
...
-- 조인
select distinct manager.ename
from emp manager
join emp staff
on manager.empno = staff.MGR;
